 ...
 * MODIFIED: Integrated ControlsManager for Plugin Widgets.
 */
...
export const Inspector = {
  ...
  renderTab(tabName) {
    ...
    // --- NEW: Handle Custom Plugin Widgets ---
    if (WidgetRegistry.has(widget.type)) {
      ...
      if (tabName === "content") {
        ...
        if (def.controls && def.controls.length > 0) {
          const controlsEl = ControlsManager.render(
            ...
            widget.settings,
            (key, value) => {
              // Map control name to settings path
              // Plugins usually store data in 'settings.key'
      ...
      // 2. Style Tab: Not yet supported for plugins in this version
      if (tabName === "style") {
        container.innerHTML =
      ...
      // 3. Advanced Tab: Falls through to the standard section definition below
    }

    // --- Existing Logic for Standard Widgets (or Advanced Tab) ---

    let sections = [];
    const def = Registry.definitions[widget.type]; // Might be undefined for custom widgets

    // 1. DEFINE SECTIONS (Groups)
    if (tabName === "advanced") {
      // Global Advanced Controls (Shared by all widgets including plugins)
      sections = [
        {
          group: "Layout",
          controls: [
